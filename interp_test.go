...
func BenchmarkInterp1e7(b *testing.B)   { benchmarkSearch(b, 1e7, SearchInts) }
func BenchmarkInterp4e7(b *testing.B)   { benchmarkSearch(b, 4e7, SearchInts) }
...
func TestSearchSmall(t *testing.T) {
	...
	for want, q := range ints {
		if idx := SearchInts(ints, q); idx != want {
